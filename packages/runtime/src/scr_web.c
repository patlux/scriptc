...
static const char web_prelude[] =
    ...
    "    _value(value, fileName, hasFileName) {\n"
    "      if (hasFileName && !(value instanceof Blob)) throw new TypeError('FormData value with a filename must be a Blob');\n"
    "      if (value instanceof Blob) {\n"
    "        if (value instanceof File && !hasFileName) return value;\n"
    "        return new File([value], hasFileName ? String(fileName) : 'blob', { type: value.type, lastModified: value.lastModified });\n"
    "      }\n"
    "      return String(value);\n"
    "    }\n"
    "    append(name, value, fileName) { this._entries.push([String(name), this._value(value, fileName, arguments.length > 2)]); }\n"
    "    delete(name) { const key = String(name); this._entries = this._entries.filter((entry) => entry[0] !== key); }\n"
    "    get(name) { const key = String(name); const entry = this._entries.find((item) => item[0] === key); return entry ? entry[1] : null; }\n"
    "    getAll(name) { const key = String(name); return this._entries.filter((entry) => entry[0] === key).map((entry) => entry[1]); }\n"
    "    has(name) { const key = String(name); return this._entries.some((entry) => entry[0] === key); }\n"
    "    set(name, value, fileName) {\n"
    "      const key = String(name);\n"
    "      const next = [key, this._value(value, fileName, arguments.length > 2)];\n"
    "      const first = this._entries.findIndex((entry) => entry[0] === key);\n"
    "      if (first < 0) { this._entries.push(next); return; }\n"
    "      this._entries[first] = next;\n"
    "      this._entries = this._entries.filter((entry, index) => index <= first || entry[0] !== key);\n"
    "    }\n"
    ...
    "    forEach(callback, thisArg) { for (const entry of this._entries) callback.call(thisArg, entry[1], entry[0], this); }\n"
    "    [Symbol.iterator]() { return this.entries(); }\n"
    ...
    "      throw new TypeError('FormData request bodies are not supported in the scriptc island');\n"
    "    }\n"
