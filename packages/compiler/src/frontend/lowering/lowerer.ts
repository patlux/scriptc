...
import { FieldTarget, lowerDynObjectLiteral, lowerExpr, maybeNarrow, lowerUnitComparison, lowerNullishCoalesce, lowerOptionalChain, finishOptionalChain, lowerCondition, ensureBool, requireTruthyUnion, eqComparableUnion, lowerIntrinsicProperty, lowerArrayLiteral, lowerObjectLiteral, lowerShorthandValue, rejectThisInObjectMethod, lowerElementAccess, lowerElementWrite, lowerNullishElementAssign, lowerNullishPropertyAssign, lowerRecordKeyRead, ensureString, lowerTemplate, lowerAsExpression, lowerPrefixUnary, lowerBinary, lowerCaughtTypeofTest, caughtRead, caughtLocalOf, caughtToString, lowerInstanceOf, lowerRegexLiteral, lowerFieldRead, lowerUnionProperty, fieldTarget, fieldGetExpr, fieldSetStmt, lowerFieldCompound, uniqueSymbolKeyOf, foldedStringKeyOf } from "./lower-exprs.js";
...
export class Lowerer {
  ...
  *classMethodMembers(
  ...
  lowerIntrinsicProperty(expr: ts.PropertyAccessExpression): IrExpr | null {
    // Builtin-spoke property extensions and the URLSearchParams generic
    // unbound method values claim their reads before the intrinsic
    // fallback's member fences fire for them.
    return lowerBuiltinExtraProperty(this, expr) ??
      lowerSearchParamsMethodValue(this, expr) ??
      lowerIntrinsicProperty(this, expr);
  }
  ...
  lowerNullishElementAssign(expr: ts.BinaryExpression): IrStmt | null {
    return lowerNullishElementAssign(this, expr);
  }

  lowerNullishPropertyAssign(expr: ts.BinaryExpression): IrStmt | null {
    return lowerNullishPropertyAssign(this, expr);
  }
